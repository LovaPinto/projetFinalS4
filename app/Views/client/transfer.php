...
<div class="content">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="title">Transfert</h1>
            <p class="subtitle">Envoyer de l'argent à un autre client</p>
        </div>
    ...
    <div class="row g-4">
        <div class="col-xl-8">
            <div class="cardx pad">
                <h2 class="h5 fw-bold mb-3">Détails du transfert</h2>
                <?= form_open(site_url('client/transfer'), ['id' => 'transferForm']) ?>
                    <div class="mb-3">
                        <label for="numero_destinataire" class="form-label fw-semibold">Numéro du destinataire</label>
                        <div class="input-group">
                            <span class="input-group-text"><i class="bi bi-telephone"></i></span>
                            <input type="tel" class="form-control" id="numero_destinataire" name="numero_destinataire"
                                   pattern="0[0-9]{9}" maxlength="10" placeholder="0379876543"
                                   required value="<?= old('numero_destinataire') ?>">
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control form-control-lg" id="montant" name="montant"
                               min="100" step="1" placeholder="10000" required
                               value="<?= old('montant') ?>">
                        <small class="text-secondary">Montant minimum : 100 Ar</small>
                    </div>
                    <button type="submit" class="btn btn-lg w-100" style="background:var(--p);color:#fff;border:0;border-radius:14px;padding:14px">
                        <i class="bi bi-send me-2"></i> Envoyer le transfert
                    </button>
                <?= form_close() ?>
            </div>
        </div>
        <div class="col-xl-4">
            <div class="cardx pad">
                ...
                <div class="d-flex justify-content-between mb-2">
                    <span class="text-secondary">Destinataire</span>
                    <b id="displayDest">-</b>
                </div>
                <div class="d-flex justify-content-between mb-2">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <b id="displayTotal" style="color:var(--p);font-size:1.2rem">0 Ar</b>
                </div>
            </div>
        </div>
    </div>
...
function calcFee(amount) {
    var schedule = feeSchedule['TRANSFERT'];
...
document.getElementById('numero_destinataire').addEventListener('input', function(){
    document.getElementById('displayDest').textContent = this.value || '-';
});
